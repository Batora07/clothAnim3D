...
void Scene::Param(std::string Fichier_Param) {
  ...
  for (unsigned int i = 1; i <= _NbObj; ++i){
    typeObjet = "objet";
    typeObjet += '0' + i;
...
void ObjetSimuleParticule::Param_particule(std::string Fichier_Param) {
  ...
  std::cout << "Utilisation du schema d integration d'Euler semi-implicite" << std::endl;
...
void ObjetSimuleMSS::Param_mss(std::string Fichier_Param) {
  ...
  if (_Integration == "explicite") {
    ...
    std::cout << "Utilisation du schema d integration d'Euler implicite" << std::endl;
