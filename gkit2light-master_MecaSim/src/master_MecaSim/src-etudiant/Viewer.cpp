 ...
 */
Viewer::Viewer(std::string* Fichier_Param, unsigned int NbObj)
  : App(1024, 768),
  mb_cullface(true), // Par defaut - gestion des faces cachees
  mb_wireframe(false), // Par defaut - affiche plein
  b_draw_grid(true), // Par defaut - affiche la grille
  ...
  for (unsigned int i = 1; i <= _Simu->_NbObj; ++i) {
    std::cout  << "Creation de l objet " << i << " de type : " << _Simu->_type_objet[i - 1] << std::endl;
...
void Viewer::help() {
  ...
  std::cout << "   c: (des)active GL_CULL_FACE" << std::endl;
  std::cout << "   w: (des)active wireframe" << std::endl;
  std::cout << "   a: (des)active l'affichage de l'axe" << std::endl;
  std::cout << "   g: (des)active l'affichage de la grille" << std::endl << std::endl;

  std::cout << "   m+fleche/pageUp/pageDown: pour bouger point interaction" << std::endl << std::endl;

  std::cout << "   fleches/pageUp/pageDown: bouge la camera"<< std::endl << std::endl;

  std::cout << "   Ctrl+fleche/pageUp/pageDown: bouge la source de lumiere" << std::endl << std::endl;

  std::cout << "   Souris+bouton gauche: rotation" << std::endl << std::endl;

  std::cout << "   Souris mouvement vertical+bouton droit: (de)zoom" << std::endl << std::endl;
}
...
int Viewer::render() {
  ...
  for (auto e = _Simu->_enfants.begin(); e != _Simu->_enfants.end(); ++e) {
    ...
    if (_Simu->_type_objet[num] == "mss") {
     ...
     // Cas systeme de particules non connectees
      ...
      for (unsigned int i = 0; i < (*e)->_Nb_Sommets; ++i) {
...
void Viewer::manageCameraLight() {
  ...
  if ((mb& SDL_BUTTON(1)) &&  (mb& SDL_BUTTON(3))) {
    ...
    m_camera.translation(static_cast<float>(mx) / static_cast<float>(window_width()),
                         static_cast<float>(my) / static_cast<float>(window_height()));
  }
