...
void ObjetSimuleMSS::initObjetSimule() {
  ...
  if (_use_texture) {
    /// Test ouverture du fichier
    if (!_FichIn_Texture) {
      std::cout << "Erreur d ouverture du fichier de donnees des textures : " << _Fich_Texture << std::endl;
...
void ObjetSimuleMSS::initMeshObjet(){
  ...
  // Boucle sur la structure P, tableau des positions
  for (unsigned int i=0;i<_VISize;i++)
  {  
      // Definition des normales
      m_ObjetSimule.normal(  
          _vectNormals[_VIndices[i]].x, 
          _vectNormals[_VIndices[i]].y, 
          _vectNormals[_VIndices[i]].z 
      ); 
      // Definition des sommets
      m_ObjetSimule.vertex(  
          P[_VIndices[i]].x, 
          P[_VIndices[i]].y, 
          P[_VIndices[i]].z 
      );        

      //m_ObjetSimule.restart_strip();
  }
