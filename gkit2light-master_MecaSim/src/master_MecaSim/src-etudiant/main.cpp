...
int main(int argc, char* argv[]) {
  ...
  if (argc == 1) {
    ...
    strcpy( argv[3], "./src/master_MecaSim/exec/Fichier_Param.objet2");
  ...
  if (argc >= 2) {
    ...
    NbObj = atoi(argv[1]);
    std::cout << "Nombre d objets dans la simulation mecanique : " << NbObj << std::endl;
    ...
    Fichier_Param = new std::string[NbObj + 1];
    ...
    std::cout << "Fichiers de donnees de la simulation : " << Fichier_Param[0] << std::endl;

    for (unsigned int i = 1; i <= NbObj; ++i)
      std::cout << "Fichier de donnees de l objet " << i << " : " << Fichier_Param[i] << std::endl;
  } else {
    /// Usage de l execution du programme
    // Exemple pour un seul objet simule :

    std::cout << "Usage depuis le repertoire gkit2light:" << std::endl;
    std::cout << "<executable> NbObj <Fichier_Param_Anim> <Fichier_Param_Obj1> <Fichier_Param_Obj2> ..." << std::endl << std::endl;

    std::cout << "Exemple pour un seul objet : " << std::endl;
    std::cout << "./bin/master_MecaSim_etudiant 1 ./src/master_MecaSim/exec/Fichier_Param.simu ./src/master_MecaSim/exec/Fichier_Param.objet1" << std::endl;
