...
public:
    ...
    void CalculPosition(unsigned int nb_som,
                        std::vector<Vector>& V,
                        std::vector<Vector>& P);
    ...
    void Allocation_Structure(unsigned int nb_som);

    /*! Initialisation des structures -
     X, Y, PP, W, W1, W2, Df_Dx, Df_Dx_diag, Df_Dv, Df_Dv_diag */
    void Init(unsigned int nb_som, MSS* _SytemeMasseRessort);

    /*! Remplissage matrices df/dx et df/dv -
     Utilisation formulation Volino ou Baraff */
    void Remplissage_df_dx_dv(unsigned int nb_som,
                              MSS * _SytemeMasseRessort,
                              std::vector<Vector> &P);

    /* Re-initialisation : Df_Dx_diag, Df_Dv_diag, Y */
    void Initialisation(unsigned int nb_som, std::vector<Vector> &Force);

    /*! Fonction construisant le vecteur Y du systeme HX = Y
     avec Y = dt f(t) + dt^2 df/dx v(t)*/
    void Remplissage_Y(unsigned int nb_som,
                       std::vector<Vector>& V,
                       std::vector<Vector>& Force,
                       std::vector<float>& M,
                       Vector gravite,
    ...
    void Resolution(unsigned int nb_som,
                    std::vector<float>& M,
                    MSS* _SytemeMasseRessort);

    /*! Calcul du produit matrice * vecteur :
     W = H * PP = (M - dt df/dv - dt^2 df/dx) * PP */
    void CalculProdMatVect(unsigned int nb_som,
                           std::vector<float>& M,
                           MSS* _SytemeMasseRessort);

    /*! Calcul de la direction PP = Y + (_alpha / _beta) * PP  */
    void CalculDirection(unsigned int nb_som);

    /*! Calcul de _beta = PP^T W */
    void CalculBeta(unsigned int nb_som);

    /*! Calcul de la solution X = X + (_alpha / _beta) * PP
     et calcul du residu Y = Y - (_alpha / _beta) * W */
    void CalculSolution_Residu(unsigned int nb_som);

    /*! Calcul de _alpha = Y^T Y */
    void CalculNorme(unsigned int nb_som);
