 ...
 */
void SolveurImpl::Allocation_Structure(unsigned int nb_som) {
  X.resize(nb_som);
  Y.resize(nb_som);
  PP.resize(nb_som);
  R.resize(nb_som);
 ...
 * X, Y, PP, W, W1, W2, Df_Dx, Df_Dx_diag, Df_Dv, Df_Dv_diag.
 */
void SolveurImpl::Init(unsigned int nb_som,
  ...
  for(unsigned int i = 0; i < nb_som; ++i) {
    ...
    const unsigned int NbRessortList_i = RessortList_i.size();

    // Pour initialiser les elements non diagonaux
    std::vector<std::vector<float>> init(NbRessortList_i);

    for (unsigned int j = 0; j < NbRessortList_i; ++j) {
      init[j] = init_element;
    }
 ...
 */
void SolveurImpl::Remplissage_df_dx_dv(unsigned int nb_som,
                                       MSS * _SystemeMasseRessort,
                                       std::vector<Vector>& P) {
 ...
 */
void SolveurImpl::Initialisation(unsigned int nb_som,
